...
def main() -> None:
    ...
    gid_all = df['game_id'].map(norm_gid)
    shooter = df['player1_name'].astype(str).str.match(r'^\s*203500(?:\.0)?\s+', na=False)
    fg = pd.to_numeric(df['is_field_goal'], errors='coerce').fillna(0).eq(1)
    regular = gid_all.str.startswith('002')
    adams = df[shooter & fg & regular].copy()
    if adams.empty:
        raise RuntimeError('No Steven Adams regular-season FGA rows in authoritative 2025-26 PBP release')
    adams['game_id'] = adams['game_id'].map(norm_gid)
    game_ids = sorted(adams['game_id'].unique().tolist())
    print(f'PBP_ROWS={len(df)} ADAMS_REGULAR_FGA_ROWS={len(adams)} ADAMS_GAMES_WITH_FGA={len(game_ids)}', flush=True)
    ...
    if errors:
        Path('adams_xfg_errors.json').write_text(json.dumps(errors, indent=2))
        raise RuntimeError(f'Official xFG requests failed for {len(errors)} Adams games; see adams_xfg_errors.json')

    allshots = pd.DataFrame(shots)
    if allshots.empty:
        raise RuntimeError('Official xFG returned zero Steven Adams shot rows')
    allshots.to_csv('adams_official_xfg_shots_2025_26.csv', index=False)
    ...
    top = made.sort_values(['xfg', 'game_date', 'game_id', 'event_id'], kind='stable').head(3).copy()
    if len(top) != 3:
        raise RuntimeError(f'Expected 3 Adams makes with official xFG and exact event IDs, got {len(top)}')
    ...
    for rank, row in enumerate(top.to_dict('records'), 1):
        ...
        events.append({
            ...
            'loc_x': row.get('loc_x'),
            'loc_y': row.get('loc_y'),
            'difficulty_definition': 'lowest official NBA shot-level xFG among Steven Adams made field goals',
            'xfg_source': 'shotqualityvideologs',
            'clip_page_url': f'https://clips.nba.com/?gameNo={gid}&eventNum={eid}&source=grs',
            'source_provenance': 'NBA shotqualityvideologs via stats.gleague.nba.com; Adams game set from authoritative ramirobentes 2025-26 PBP release asset; regular season constrained to 002 game IDs',
        })

    request = {
        'query': 'Steven Adams 3 hardest made field goals, 2025-26 regular season, ranked by lowest exact official NBA shot-level xFG',
        'player_id': PLAYER_ID,
        'season': SEASON,
        'season_type': 'Regular Season',
        'ranking': {'field': 'xfg', 'direction': 'ascending', 'made_only': True, 'take': 3},
        'pbp_release_url': PBP_URL,
        'xfg_source': XFG_URL,
        'adams_regular_fga_rows': len(adams),
        'adams_games_with_fga': len(game_ids),
    ...
    print('TOP3=' + json.dumps(events), flush=True)
