...
def main():
    ...
    for e in sorted(events,key=lambda x:x['rank']):
        ...
        plans.append({'rank':e['rank'],'game_id':gid,'event_id':eid,'matchup':e.get('matchup'),'game_date':e.get('game_date'),'hybrid_difficulty_score':e.get('hybrid_difficulty_score'),'angle_count':len(opts),'angle_labels':[o['label'] for o in opts]})
    ...
    for r in results:
        if r['status']=='ok': sha[r['probe']['sha256']].append(r); fp[tuple(r['probe']['visual_fingerprints'])].append(r)
    bad=set()
    for group in list(sha.values())+list(fp.values()):
        ev={(r['game_id'],r['event_id']) for r in group}
        if len(ev)>1: bad|=ev
    for r in results:
        if (r['game_id'],r['event_id']) in bad: r['status']='failed'; r['global_qa_failure']='duplicate_media_across_distinct_events'
    ...
    payload={'mode':'kd_top10_all_camera_angles_source','policy':'Same validated KD hybrid-v3 top 10. For every ranked event, include every clips.nba.com lrmedia camera option; each angle uses its highest native HLS rendition. No scaling/re-encode in source clips.','request':req,'shot_plans':plans,'expanded_clip_count':len(results),'valid_clip_count':len(ok),'clips':results}
