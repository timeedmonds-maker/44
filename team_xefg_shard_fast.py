...
def fetch_pair(game_id: str, player_id: int, max_attempts: int, connect_timeout: float, read_timeout: float):
    params = {'GameID': game_id, 'PlayerID': int(player_id)}
    last = None
    for attempt in range(1, max_attempts + 1):
        started = time.time()
        try:
            r = session().get(XFG_URL, params=params, timeout=(connect_timeout, read_timeout))
...
def main():
    ...
    ap.add_argument('--out-dir', default='team_xefg_fast')
    args = ap.parse_args()
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as ex:
        futs = {
            ex.submit(fetch_pair, str(r.game_id), int(r.player_id), args.max_attempts, args.connect_timeout, args.read_timeout):
