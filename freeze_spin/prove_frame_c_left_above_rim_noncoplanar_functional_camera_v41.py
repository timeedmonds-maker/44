...
RIM_ROI={40:[450,505,120,145],220:[465,510,155,175],440:[455,505,180,200],620:[455,505,165,185]}
...
def main():
 ...
 obs={k:np.array(v,float) for k,v in wide['observations_px'].items()};held={k:set(v) for k,v in wide['held_out_indices'].items()}
 def metric_res(core,obsx=obs,rimc=TARGET_RIM_CENTER):
  ...
  z.extend(((project(C,pp,par,RIM_CENTER)-np.asarray(rimc).reshape(1,2))*3).ravel());return np.asarray(z)
 ...
 rimerr=float(np.linalg.norm(project(C,pp,par,RIM_CENTER)-TARGET_RIM_CENTER.reshape(1,2)))
 static=[];off=6
 for pr in sett:
 ...
 gates={'center_root_spread':center_spread<=1.0,'center_loo':max(x['center_shift_cm'] for x in loo.values())<=15.0,'target_floor_heldout':float(np.percentile(ho,95))<=1.5,'target_rim_center':rimerr<=1.0,'target_static_heldout':max(x['p95_px'] for x in static)<=3.0,'functional_root_p95':fe95<=0.5,'functional_root_max':femax<=0.75}
 passed=all(gates.values())
 rep={'schema_version':1,'status':'PASS_NONCOPLANAR_FUNCTIONAL_CAMERA_V41' if passed else 'FAIL_NONCOPLANAR_FUNCTIONAL_CAMERA_V41','game_id':'0022500301','event_id':489,'camera_label':'Left Above Rim','method':'independent same-game v35 floor transfers + regulation rim-circle pixels solve physical centre; held-out Frame C raw wide-court + rim-centre + settled static background; multistart accepted only if functionally sub-pixel equivalent across 3D action volume','independent_events':EVENTS,'selected_samples':{str(e):Path(sel[e]['source']).name for e in EVENTS},'camera_center_cm':C.tolist(),'game_level_pp_diagnostic_px':gamepp.tolist(),'center_multistart_max_cm':center_spread,'game_pp_multistart_max_px':gamepp_spread,'leave_one_event_out':loo,'target_camera':{'rvec':core0[:3].tolist(),'focal_px':float(np.exp(core0[3])),'principal_point_px':core0[4:6].tolist(),'parameter_pp_root_spread_px':float(pp_root)},'target_metric':{'floor_train_p95_px':float(np.percentile(tr,95)),'floor_heldout_p95_px':float(np.percentile(ho,95)),'rim_center_error_px':rimerr},'settled_static':{'count':len(sett),'median_initial_source_focal_px':med,'heldout':static},'functional_root_equivalence':{'action_volume_cm':{'x':[-30,250],'y':[-180,180],'z':[20,350]},'max_p95_px':fe95,'max_px':femax,'pairs':fe},'gates':gates,'permissions':{'physical_camera_center_allowed':passed,'metric_event_camera_allowed':passed,'replay_render_allowed':False},'retired_constraint':'legacy backboard inner-target pixels are diagnostic only and excluded from v41 fit'}
