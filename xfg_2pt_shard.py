...
def main() -> None:
    ...
    ap.add_argument('--out-dir', default='xfg_2pt_shards')
    args = ap.parse_args()
    ...
    def apply_payload(pid: int, payload: dict) -> None:
        ...
        for s in shot_list:
            ...
            shot_type = str(s.get('shotType') or '').upper().strip()
            is3 = shot_type.startswith('3PT')
            a['tracked_fga'] += 1
            if not is3:
                a['tracked_2pa'] += 1
                a['tracked_2pm'] += made
                a['expected_2pt_makes'] += xfg
    ...
    for pid, a in acc.items():
        ...
        rows.append({
            ...
            'tracked_2pa': int(a['tracked_2pa']),
            'tracked_2pm': int(a['tracked_2pm']),
            'expected_2pt_makes': float(a['expected_2pt_makes']),
