...
Durable deterministic screen-analysis renderer derived from the recovered Christmas Day V7 lineage.
No generated imagery. No AI super-resolution. Real source pixels + deterministic OpenCV/Pillow overlays only.

Version lock:
- visual lineage: repo 44 / codex/adams-screen-okc-opener / Christmas event 94 V7
- recovered successful run: 34826047210
- recovered commit: 40d0ed3974ecbd0fcd433670cddd462c26f9e397
- analytics panels: neutral_translucent_grey_v1
- ring colors: exact team primary RGB supplied in config
- release shot-distance tag: parsed from authoritative shot-description text in config, never inferred from image geometry
"""
...
TOOL_ID='LOCKED_BROADCAST_SCREEN_V1'
PANEL_FILL=(70,70,72)
...
def render(source,tracks,config,out):
    ...
    cstart=float(cfg['timing'].get('contact_start_s',release)); cend=float(cfg['timing'].get('contact_end_s',release))
    ...
    while frame_idx<ef:
        ...
        if t>=cstart:
            elapsed=max(0.0,min(t,cend)-cstart); fr=draw_contact(fr,W-222,87,202,elapsed)
        if cfg['shot'].get('league_xfg_pct') is not None and release-1.0<=t<=release+2.0:
            fr=draw_tile(fr,20,20,194,66,'SHOT QUALITY',f"xFG  {float(cfg['shot']['league_xfg_pct']):.1f}%",27)
    ...
    qa={'tool_id':TOOL_ID,'visual_lineage':{'repo':'timeedmonds-maker/44','branch':'codex/adams-screen-okc-opener','run_id':34826047210,'commit':'40d0ed3974ecbd0fcd433670cddd462c26f9e397','baseline':'Christmas event 94 V7'},'deterministic_only':True,'ai_image_generation':False,'ai_super_resolution':False,'shot_description':shot_desc,'shot_distance_tag':dist_label,'shot_distance_source':'shot_description_text_regex','team_primary_rgb':{k:v['primary_rgb'] for k,v in cfg['teams'].items()},'panel_style':'neutral_translucent_grey_v1','native':str(native),'uhd':str(uhd),'release_s':release,'freeze_s':freeze}
